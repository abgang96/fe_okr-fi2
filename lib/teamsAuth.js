...
export const teamsAuth = {
   ...
   */  async getTeamsToken() {
    try {
      ...
      // Detect macOS for special handling
      ...
      try {
        ...
        console.log(`Teams client type detected: ${clientType}, macOS: ${isMacOS}`);
      } catch (contextError) {
      ...
      if (isMacOS) {
        console.log('Detected macOS platform, using specialized approach');
        try {
          // For macOS, use the callback approach which tends to work better
          const macToken = await new Promise((resolve, reject) => {
            ...
            authentication.authenticate({
              ...
              successCallback: (result) => {
                ...
                console.log('macOS auth success');
                resolve(result);
              },
              failureCallback: (reason) => {
                clearTimeout(timeout);
                console.error('macOS auth failure:', reason);
                reject(new Error(reason || "Authentication failed"));
              }
            });
          });
          
          console.log('macOS authentication succeeded');
          return macToken;
        } catch (macError) {
          console.error('macOS authentication failed:', macError);
          throw macError; // Allow fallback to other methods
        }
      }
      
      // For non-macOS platforms, first try silent token acquisition
      try {
        ...
        // For desktop client, try the authenticate method with modern Promise approach
